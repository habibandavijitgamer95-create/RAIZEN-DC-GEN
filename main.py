...
# Render Error Fix: exist_ok=True fixes FileExistsError
...
class MyBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        print(f"🚀 Raizen Master Gen Online!")
...
async def redeem(interaction: discord.Interaction, code: str):
    ...
    if row:
        ...
        try:
            ...
            await interaction.response.send_message(f"✅ Success! Category: **{tier}**, Service: **{service}**.", ephemeral=True)
        except:
...
async def leaderboard(interaction: discord.Interaction):
    ...
    for i, row in enumerate(rows, 1):
        ...
        lb += f"**{i}.** {user} — `{row[1]}` gens\n"
    embed = discord.Embed(title="🏆 Top 10 Generators", description=lb or "No data yet", color=0xFFD700)
    await interaction.response.send_message(embed=embed)
...
@bot.tree.command(name="send", description="Send account directly via DM")
async def send(interaction: discord.Interaction, member: discord.Member, service: str):
    if interaction.user.id not in STAFF_IDS: return await interaction.response.send_message("Staff only!", ephemeral=True)
    path = f"{BASE_PATH}/{service.capitalize()}.txt"
    if not os.path.exists(path) or get_count(service) == 0:
        return await interaction.response.send_message("Out of stock!", ephemeral=True)
    with open(path, "r") as f: lines = f.readlines()
    acc = lines.pop(0).strip()
    with open(path, "w") as f: f.writelines(lines)
    try:
        await member.send(f"🎁 **Special Gift!**\nService: {service}\nAccount: `{acc}`")
        await interaction.response.send_message(f"✅ Sent to {member.mention}")
    except: await interaction.response.send_message("❌ DM Closed!")

@bot.tree.command(name="addv", description="Add vouches manually")
